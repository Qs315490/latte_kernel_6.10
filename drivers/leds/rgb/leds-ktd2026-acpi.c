...
static const struct acpi_device_id ktd2026_acpi_match[] = {
	{KTD_ACPI_NAME, 0},
	{},
};
MODULE_DEVICE_TABLE(acpi, ktd2026_acpi_match);
...
static struct i2c_driver ktd2026_driver = {
	.driver = {
		.name	= KTD_I2C_NAME,
		.owner = THIS_MODULE,
		.acpi_match_table = ACPI_PTR(ktd2026_acpi_match),
	},
	.probe = ktd2026_probe,
...
static int __init ktd2026_init(void)
{
	int err;
	printk("Enter:%s\n", __func__);
	err = i2c_add_driver(&ktd2026_driver);
	if (err) {
		printk(KERN_ERR "ktd2026-acpi driver failed "
				"(errno = %d)\n", err);
	} else {
		printk("Successfully added driver %s\n",
				ktd2026_driver.driver.name);
	}
	return err;
}
